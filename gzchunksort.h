...
void GzChunkSortWriter<K>::write_lines() {
	...
	for (typename map<K, vector<string>>::iterator it = this->sorted_lines.begin(); it != this->sorted_lines.end(); ++it) {
...
void GzChunkSortWriter<K>::write_line(K key, string line) {
	...
	if (this->sorted_lines.find(key) != this->sorted_lines.end()) {
		...
		this->sorted_lines.insert(make_pair(key, lvect)) ;
	}
...
int collect_sorted_chunks(const string& out_file_path_str, 
	const vector<string> file_vect, const function<K (string)> key_getter) {
    ...
    while(gzin_vect.size() > 1) {
        ...
        // get the low key
        K low_key = keys.at(0) ;
        for (K key : keys) { if (key < low_key) low_key = key ; }
